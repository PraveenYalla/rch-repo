import { Pressable, StyleSheet, Text, View, ScrollView, Button, TextInput, Alert, Keyboard, DeviceEventEmitter } from 'react-native'
...
const EditDetails = ({ ...props }) => {
    ...
    const { item: record } = props.route.params;
    ...
    const [rchId, setrchId] = useState(record.rch_id_no);
    const [healhProviderName, sethealhProviderName] = useState(record.health_provider_name);
    const [ashaName, setashaName] = useState(record.asha_name);
    const [deathDate, setDeathDate] = useState(record.death_date);
    const [ancSite, setancSite] = useState(record.site_of_ANC_done);
    const [placeName, setplaceName] = useState(record.place_name);
    const [ancDate, setancDate] = useState(record.ANC_date);
    const [weightPW, setweightPW] = useState(record.weight_of_PW);
    const [causeOfDeath, setcauseOfDeath] = useState(record.probable_cause_of_death);
    const [maternalDeath, setmaternalDeath] = useState(record.maternal_Death);

    const [maternalStatus, setmaternalStatus] = useState(record.maternal_Death === "Yes" ? true : false);
    ...
    const updateDetails = async () => {
        ...
        const getFiltered = data?.map((d) => {
            if (d.rch_id_no === record.rch_id_no) {
                d = obj
            }
            return d;
        })
        ...
        await AsyncStorage.setItem('anclist', JSON.stringify(getFiltered));
        ...
        <>
            <ScrollView style={{ flexDirection: "column" }}>
                <View style={{ margin: 20, gap: 20 }}>
                    <View>
                        <Text>RCH ID No: {record.rch_id_no}</Text>
                    ...
                    <View>
                        ...
                        <Text class="fingerbox">
                            <Button title="Capture Finger4" id="finger4" onPress={async () => {
                                ...
                                Alert.alert('Success', isAvailable, [
                                    { text: 'OK', onPress: () => console.log('OK Pressed') },
                                ]);
                                // FingerprintScanner
            ...
            <View style={{ marginTop: "auto", padding: 20, backgroundColor: "#eee", flexDirection: "row" }}>
                <Pressable style={{ padding: 20, alignItems: "center", marginRight: 10, backgroundColor: disableBtn ? '#607D8B' : '#003580' }} onPress={updateDetails} disabled={disableBtn}>
